...
Supported schemes are HTTP authentication, an API key (either as a header, a cookie parameter or as a query parameter), OAuth2's and OpenID's common flows (implicit, password, client credentials, authorization code and ciba) as defined in [RFC6749](https://tools.ietf.org/html/rfc6749), [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html) and [OpenID Connect Client-Initiated Backchannel Authentication](https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html)).

##### Fixed Fields
Field Name | Type | Applies To | Description
---|:---:|---|---
<a name="securitySchemeType"></a>type | `string` | Any | **REQUIRED**. The type of the security scheme. Valid values are `"apiKey"`, `"http"`, `"oauth2"`, `"ciba"`, `"openIdConnect"`.
...
<a name="securitySchemeFlows"></a>flows | [OAuth Flows Object](#oauthFlowsObject) | `oauth2` `openIdConnect` `ciba` | **REQUIRED**. An object containing configuration information for the flow types supported.
...
<a name="oauthFlowsCiba"></a>ciba| [OAuth Flow Object](#oauthFlowObject) | Configuration for the OpenID Connect Client-Initiated Backchannel Authentication Flow.
...
Field Name | Type | Applies To | Description
---|:---:|---|---
<a name="oauthFlowAuthorizationUrl"></a>authorizationUrl | `string` | `oauth2` (`"implicit"`, `"authorizationCode"`, `"ciba"`) | **REQUIRED**. The authorization URL to be used for this flow. This MUST be in the form of a URL.
<a name="oauthFlowTokenUrl"></a>tokenUrl | `string` | `oauth2` (`"password"`, `"clientCredentials"`, `"authorizationCode"`, `"ciba"`) | **REQUIRED**. The token URL to be used for this flow. This MUST be in the form of a URL.
...
<a name="oauthFlowCiba"></a>x-ciba-request-parameters | Map[`string`, `string`] | `ciba` | **REQUIRED**. CIBA authentication request parameters. A map between the request parameter key and its value. The map MAY be empty.
...
Field Pattern | Type | Description
---|:---:|---
<a name="securityRequirementsName"></a>{name} | [`string`] | Each name MUST correspond to a security scheme which is declared in the [Security Schemes](#componentsSecuritySchemes) under the [Components Object](#componentsObject). If the security scheme is of type `"oauth2"` or `"openIdConnect"` or `"ciba"`, then the value is a list of scope names required for the execution, and the list MAY be empty if authorization does not require a specified scope. For other security scheme types, the array MUST be empty.
